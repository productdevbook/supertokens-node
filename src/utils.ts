...
export function maxVersion(version1: string, version2: string): string {
  const splittedv1 = version1.split('.')
  const splittedv2 = version2.split('.')
  const minLength = Math.min(splittedv1.length, splittedv2.length)
  for (let i = 0; i < minLength; i++) {
    const v1 = Number(splittedv1[i])
    const v2 = Number(splittedv2[i])
    if (v1 > v2)
      return version1
    else if (v2 > v1)
      return version2
...
export function getTopLevelDomainForSameSiteResolution(url: string): string {
  ...
  const parsedURL = psl.parse(hostname) as psl.ParsedDomain
