...
import { APIInterface, APIOptions, TypeProvider, User } from '../'
  ...
  return {
    async authorisationUrlGET({
      ...
      userContext: any
        ...
        > {
      const providerInfo = provider.get(undefined, undefined, userContext)

      const params: { [key: string]: string } = {}
      const keys = Object.keys(providerInfo.authorisationRedirect.params)
      for (let i = 0; i < keys.length; i++) {
        const key = keys[i]
        const value = providerInfo.authorisationRedirect.params[key]
    ...
    async signInUpPOST({
      ...
      const providerInfo = provider.get(redirectURI, code, userContext)
      ...
      const response = await options.recipeImplementation.signInUp({
    ...
    async appleRedirectHandlerPOST({ code, state, options }): Promise<void> {
      const redirectURL
