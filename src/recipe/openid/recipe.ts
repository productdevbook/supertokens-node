...
export default class OpenIdRecipe extends RecipeModule {
  ...
  constructor(recipeId: string, appInfo: NormalisedAppinfo, isInServerlessEnv: boolean, config?: TypeInput) {
    ...
    const builder = new OverrideableBuilder(RecipeImplementation(this.config, this.jwtRecipe.recipeInterfaceImpl))

    this.recipeImplementation = builder.override(this.config.override.functions).build()

    const apiBuilder = new OverrideableBuilder(APIImplementation())
